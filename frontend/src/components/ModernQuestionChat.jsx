...
const ModernQuestionChat = ({ isOpen, onClose, questionId, questionTitle }) => {
  ...
  const loadMessages = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://plusdsa.onrender.com/api'}/question-chat/${questionId}`);
      if (response.ok) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://plusdsa.onrender.com/api'}/question-chat/${questionId}/send`, {
        method: 'POST',
