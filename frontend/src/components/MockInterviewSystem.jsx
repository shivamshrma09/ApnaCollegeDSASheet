...
const MockInterviewSystem = ({ isDark, onClose }) => {
  ...
  const generateQuestionsWithGemini = async (formData) => {
    ...
    try {
      ...
      const backendResponse = await fetch(`${import.meta.env.VITE_API_URL}/mock-interview/start`, {
        method: 'POST',
        headers: {
          'userid': 'anonymous'
        },
  ...
  const handleInterviewComplete = async (answers) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/mock-interview/submit`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
