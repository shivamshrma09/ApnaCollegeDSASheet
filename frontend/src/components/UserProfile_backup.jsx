...
// import { dsaProblemsData } from '../data/problems'; // File removed

// Simple replacement data
const dsaProblemsData = [
  {
    id: 1,
    title: 'Arrays',
    problems: [
      { id: 1, title: 'Two Sum', difficulty: 'Easy', link: '#', video: '#' },
      { id: 2, title: 'Best Time to Buy and Sell Stock', difficulty: 'Easy', link: '#', video: '#' }
    ]
  },
  {
    id: 2,
    title: 'Strings',
    problems: [
      { id: 3, title: 'Valid Anagram', difficulty: 'Easy', link: '#', video: '#' },
      { id: 4, title: 'Valid Parentheses', difficulty: 'Easy', link: '#', video: '#' }
    ]
  }
];
...
const UserProfile = ({ isOpen, onClose, userId }) => {
  ...
  const fetchUserRank = async () => {
    try {
      ...
      const completed = completedProblems.size;
      setUserRank({ rank: Math.max(1, 100 - completed), totalUsers: 1000 });
    }
  };
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px', marginBottom: '24px' }}>
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#06b6d4', marginBottom: '6px' }}>
                      #{leaderboard.findIndex(u => u._id === userId) + 1 || userRank?.rank || '---'}
                    </div>
                    ...
                    >
                      ...
                      <div style={{ maxHeight: '120px', overflowY: 'auto' }}>
                        {Array.from(starredProblems).slice(0, 5).map(problemId => {
                          // Find problem details from appropriate data source
                          let problemDetails = null;
                          const dataSource = selectedSheet === 'loveBabbar' ? [] : dsaProblemsData; // Will be loaded dynamically
                          for (const topic of dataSource) {
                            const problem = topic.problems?.find(p => p.id === parseInt(problemId));
                            if (problem) {
                          ...
                          return (
                            <div key={problemId} style={{
                              ...
                              <div style={{ color: isDark ? '#d1d5db' : '#374151', fontWeight: '500' }}>
                                {problemDetails?.title || `Problem #${problemId}`}
                              </div>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'grid', gap: '8px', maxHeight: '300px', overflowY: 'auto' }}>
                    {loading ? (
                      ...
                      leaderboard
                        .sort((a, b) => {
                          const aSolved = a.totalSolved || a.problemsSolved || 0;
                          const bSolved = b.totalSolved || b.problemsSolved || 0;
                          return bSolved - aSolved;
                        })
                        .slice(0, 10)
                        .map((leaderUser, index) => {
                          const isCurrentUser = leaderUser._id === userId || leaderUser.name === user.name;
                          const actualRank = index + 1;
                          const solvedCount = leaderUser.totalSolved || leaderUser.problemsSolved || 0;
                          return (
                            <div key={leaderUser._id} style={{
                              display: 'flex',
                              alignItems: 'center',
                              gap: '12px',
                              padding: '12px',
                              ...
                              <div style={{ flex: 1 }}>
                                ...
                                <div style={{ color: isDark ? '#9ca3af' : '#6b7280', fontSize: '12px' }}>
                                  {solvedCount} problems ({selectedSheet === 'apnaCollege' ? 'Apna College' : selectedSheet === 'loveBabbar' ? 'Love Babbar' : selectedSheet === 'striverA2Z' ? 'Striver A2Z' : selectedSheet === 'striverSDE' ? 'Striver SDE' : selectedSheet === 'blind75' ? 'Blind 75' : selectedSheet === 'striverBlind75' ? 'Striver Blind 75' : selectedSheet === 'neetcode' ? 'NeetCode 150' : 'Apna College'}) • Joined {new Date(leaderUser.joinedAt).toLocaleDateString()}
                                </div>
                              </div>
                  ...
                  {/* User's Rank Info */}
                  {!leaderboard.slice(0, 10).some(u => u._id === userId) && userRank && (
                    <div style={{
                      marginTop: '16px',
                      padding: '12px',
                      ...
                      <div style={{ color: '#3b82f6', fontWeight: '600', fontSize: '14px' }}>
                        Your Rank: #{userRank.rank} out of {userRank.totalUsers} users
                      </div>
                      <div style={{ color: isDark ? '#9ca3af' : '#6b7280', fontSize: '12px', marginTop: '4px' }}>
                        {userRank.problemsSolved || progress.completed} problems solved
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
                  ...
                  {[
                    ...
                    ['today', 'day1', 'day3', 'week1', 'week2', 'month1', 'month3'].map(key => {
                      const problems = forgettingCurveData[key] || [];
                      const problemIds = problems.map(p => typeof p === 'object' ? p.problemId : p).filter(Boolean);
                      
                      return {
                        name: (key === 'today' ? 'Review Today' : 
                              key === 'day1' ? 'Tomorrow' :
                              key === 'day3' ? 'Day 3' :
                              key === 'week1' ? 'Week 1' :
                              key === 'week2' ? 'Week 2' :
                              key === 'month1' ? 'Month 1' :
                              key === 'month3' ? 'Month 3' : key) + ` (${getSheetDisplayName(selectedSheet)})`,
                        icon: key === 'today' ? <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M12,4A8,8 0 0,1 20,12A8,8 0 0,1 12,20A8,8 0 0,1 4,12A8,8 0 0,1 12,4M12.5,7V12.25L17,14.92L16.25,16.15L11,13V7H12.5Z"/></svg> : 
                              key === 'day1' ? <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6,6.9L3.87,4.78L5.28,3.37L7.4,5.5L6,6.9M13,1V4H11V1H13M21,11V13H18V11H21M4.5,10.5V12.5H1.5V10.5H4.5M15.07,6.93L17.2,4.8L18.61,6.21L16.48,8.34L15.07,6.93M12,7A5,5 0 0,1 17,12A5,5 0 0,1 12,17A5,5 0 0,1 7,12A5,5 0 0,1 12,7M12,9A3,3 0 0,0 9,12A3,3 0 0,0 12,15A3,3 0 0,0 15,12A3,3 0 0,0 12,9M6,20H18A2,2 0 0,1 20,22H4A2,2 0 0,1 6,20Z"/></svg> :
                              key === 'day3' ? <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M19,3H18V1H16V3H8V1H6V3H5A2,2 0 0,0 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5A2,2 0 0,0 19,3M19,19H5V8H19V19Z"/></svg> :
                              key === 'week1' ? <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M3,3V21H21V19H5V3H3M22,7H6V9H22V7M22,11H6V13H22V11M22,15H6V17H22V15Z"/></svg> :
                              key === 'week2' ? <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M16,6L18.29,8.29L13.41,13.17L9.41,9.17L2,16.59L3.41,18L9.41,12L13.41,16L19.71,9.71L22,12V6H16Z"/></svg> :
                              key === 'month1' ? <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M9,10V12H7V10H9M13,10V12H11V10H13M17,10V12H15V10H17M19,3A2,2 0 0,1 21,5V19A2,2 0 0,1 19,21H5C3.89,21 3,20.1 3,19V5A2,2 0 0,1 5,3H6V1H8V3H16V1H18V3H19M19,19V8H5V19H19M9,14V16H7V14H9M13,14V16H11V14H13M17,14V16H15V14H17Z"/></svg> :
                              key === 'month3' ? <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M19,19H5V8H19M16,1V3H8V1H6V3H5C3.89,3 3,3.89 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5C21,3.89 20.1,3 19,3H18V1M17,12H12V17H17V12Z"/></svg> : <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2Z"/></svg>,
                        count: problemIds.length,
                        color: key === 'today' ? '#ef4444' : 
                               key === 'day1' ? '#f59e0b' :
                               key === 'day3' ? '#3b82f6' :
                               key === 'week1' ? '#8b5cf6' :
                               key === 'week2' ? '#06b6d4' :
                               key === 'month1' ? '#22c55e' :
                               key === 'month3' ? '#ec4899' : '#6b7280',
                        problems: problemIds,
                        type: 'forgetting',
                        desc: (key === 'today' ? 'Due for review now' :
                              key === 'day1' ? 'Review tomorrow' :
                              key === 'day3' ? 'Review in 3 days' :
                              key === 'week1' ? 'Review next week' :
                              key === 'week2' ? 'Review in 2 weeks' :
                              key === 'month1' ? 'Review next month' :
                              key === 'month3' ? 'Review in 3 months' : 'Spaced repetition') + ` - ${getSheetDisplayName(selectedSheet)} sheet`
                      };
                    })
                  ).map((playlist, index) => (
                    <div key={index} style={{
...
const PlaylistModal = ({ playlist, onClose, isDark }) => {
  const { completedProblems, starredProblems, notes, playlists, toggleStar, saveNote, addToPlaylist } = useUserProgress(JSON.parse(localStorage.getItem('user') || '{}').id);
  ...
  useEffect(() => {
    const loadProblemsData = async () => {
      try {
        // Determine which sheet data to load based on playlist name
        const isLoveBabbar = playlist.name && playlist.name.includes('Love Babbar');
  ...
  const getProblemsForPlaylist = () => {
    const findProblemById = (id) => {
      ...
      // Return a placeholder if problem not found
      return {
        id: problemId,
    ...
    if (playlist.type === 'starred' || playlist.type === 'notes' || playlist.type === 'forgetting') {
            ...
            <div>
                        ...
                        {problems.map((problem, index) => (
                          <div key={problem.id} className="problem-row" style={{display: 'grid', gridTemplateColumns: '60px 1fr 80px 80px 60px 80px 60px 80px 80px 80px 80px 80px', gap: '12px', padding: '12px 24px', borderBottom: `1px solid ${isDark ? '#4b5563' : '#f1f3f4'}`, alignItems: 'center', fontSize: '14px', backgroundColor: isDark ? '#1f2937' : 'white'}}>
                            {/* Status */}
                            <div className="problem-status" style={{display: 'flex', justifyContent: 'center'}}>
...
// Helper function to get top language from repos
const getTopLanguage = (repos) => {
  const languages = {};
  repos.forEach(repo => {
    if (repo.language) {
  ...
  try {
    ...
    if (token) {
      await fetch('http://localhost:5001/api/platforms/save', {
        ...
        body: JSON.stringify({ platform, username, data })
      });
    }
  } catch (error) {
...
// Certificate download function
const downloadCertificate = (userName, progress, sheetType) => {
  if (((progress.completed / progress.total) * 100) < 80) {
    alert('You need to complete at least 80% of problems to download certificate');
    return;
  }
  ...
  ctx.font = '20px Arial';
  ctx.fillStyle = '#1f2937';
  ctx.fillText(`has successfully completed ${progress.completed} out of ${progress.total} problems`, 400, 300);
  ctx.fillText(`in the ${sheetType === 'apnaCollege' ? 'Apna College' : sheetType} DSA Sheet`, 400, 330);
  ctx.fillText(`with ${((progress.completed / progress.total) * 100).toFixed(1)}% completion rate`, 400, 360);
  
  // Date
  ctx.font = '16px Arial';
  ctx.fillStyle = '#6b7280';
  ctx.fillText(`Issued on ${new Date().toLocaleDateString()}`, 400, 450);
  
  // Download
  const link = document.createElement('a');
  link.download = `${userName}_${sheetType}_Certificate.png`;
  link.href = canvas.toDataURL();
  link.click();
};

// Share Playlist Modal Component
const SharePlaylistModal = ({ playlist, onClose, isDark }) => {
  const [shareLink, setShareLink] = useState('');
  const [copied, setCopied] = useState(false);
  
  useEffect(() => {
    // Generate a shareable link (in real app, this would be a backend endpoint)
    const link = `${window.location.origin}/playlist/${playlist.id}?share=true`;
    setShareLink(link);
  }, [playlist]);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h4 style={{ color: isDark ? 'white' : '#1f2937', marginBottom: '16px' }}>Share Playlist</h4>
        <p style={{ color: isDark ? '#9ca3af' : '#6b7280', marginBottom: '16px' }}>
          Share "{playlist.name}" with others
        </p>
        
        <div style={{ marginBottom: '16px' }}>
          <label style={{ color: isDark ? '#d1d5db' : '#374151', fontSize: '14px', fontWeight: '500' }}>Share Link</label>
        ...
        <div style={{ color: isDark ? '#9ca3af' : '#6b7280', fontSize: '12px', marginBottom: '16px' }}>
          💎 Premium Feature: Share playlists with friends and collaborate on problem solving!
        </div>
